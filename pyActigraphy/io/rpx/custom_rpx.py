...
class CustomRawRPX(BaseRaw):
    ...
    def __init__(
        ...
        def __dequote(s):
            ...
            if (len(s) >= 2 and s[0] == s[-1]) and s.startswith(("'", '"')):
                return s[1:-1]
            return s
        data = list(map(__dequote, data))

        # read file header info
        header_offset, data_offset, header, data_available_cols = \
            self.__extract_rpx_header_info(data, delimiter)
    ...
    def __extract_rpx_header_info(self, data, delimiter):
        ...
        for data_offset, line in enumerate(data[header_offset+1:]):
            if line == empty_line:
                ...
                data_available_cols.append(
                    line.split(delimiter)[0].strip('"').rstrip(':')
                )
